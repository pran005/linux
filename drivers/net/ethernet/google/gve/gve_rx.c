			       ...
			       union gve_rx_data_slot *data_slot)
{
	...
	WARN_ON(page_info->pagecnt_bias != 1);
	gve_free_page(dev, page_info->netmem, dma, DMA_FROM_DEVICE);
}
			       ...
			       struct gve_rx_ring *rx)
{
	...
	err = gve_alloc_page(priv, dev, &netmem, &dma, DMA_FROM_DEVICE,
			     GFP_ATOMIC, NULL);
	if (err) {
					   ...
					   u16 len, struct napi_struct *napi)
{
	...
	if (copy_page_info->can_flip) {
		...
		rx->qpl_copy_pool_head++;
		prefetch(netmem_to_page(rx->qpl_copy_pool[rx->qpl_copy_pool_head & rx->qpl_copy_pool_mask].netmem));
	} else {
		   ...
		   struct gve_rx_cnts *cnts)
{
	...
	prefetch(netmem_to_page(page_info->netmem)); /* Kernel page struct. */
